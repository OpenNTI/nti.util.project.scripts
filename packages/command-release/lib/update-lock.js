...
export async function usesLock (dir) {
	return isTrue(await exec(dir, 'npm config get package-lock')) || isTrue(await exec(dir, 'npm config get package-lock-releases'));
}
...
export async function updateLock (dir, dryRun) {
	...
	if (usesLocks) {
		if (dryRun) {
			...
			write('[dry run]:       or: "npm config get package-lock-releases": true');
		} else {
