...
const isWorker =
	LINTER ||
	process.argv.some(x => /thread-loader.*worker/gi.test(x)) ||
	process.env.NTI_BROWSER_LIST_PRINTED != null;
...
if (!isWorker && (process.stdout.isTTY || isCI)) {
	...
	const selected = Object.keys(browsers)
		.map(combine)
		.sort(byLocale)
		.join('\n  ');
...
`
		...
		? ''
		: `
Default targets are defined in ${chalk.grey(
				'@nti/lib-scripts/config/browserlist'
		  )}.
