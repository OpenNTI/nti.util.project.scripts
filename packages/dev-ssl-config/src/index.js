...
import getDockerCert from './docker.js';
...
async function getCertPaths() {
	...
	const { KEY = CA_ROOT + '.key', CERT = CA_ROOT + '.crt' } = DOCKER || {};
	return {
		KEY,
		CERT,
	};
...
export async function getSSLFlags() {
	...
	[KEY, CERT] = await Promise.all(
		[KEY, CERT].map(async file =>
			/^https?:\/\//i.test(file)
				? getTempFile(await readFile(file))
				: file
		)
	);

	return ['--https', '--key', KEY, '--cert', CERT];
}
...
(async function validate() {
	...
	{dim ) Start the docker containers and/or make sure http://localhost responds to requests.} ${
		DOCKER ? '' : chalk`{dim (currently }{bold.dim not} {dim responding)}`
	}
