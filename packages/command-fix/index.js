...
const getMaintBranchName = v => (
	(v = semver.parse(v)),
	v && ((v.patch = ''), `maint-${v.format().replace(/\.$/, '')}`)
);
...
const branches = toList(call('git', 'branch', '-a')).map(x =>
	x.replace(/^[*\s]+/, '')
);
...
function handleResponse({ fix = list[0] }) {
	...
	if (localBranch) {
		...
		write(
			chalk.cyan(
				`Creating a new local branch starting at: ${chalk.underline.magenta(
					tag
				)}`
			)
		);
		...
		if (!remoteBranch) {
			write(
				chalk.cyan(
					'No remote branch detected, pushing new branch to remote...'
				)
			);
			...
			write(
				chalk.cyan(
					'Exiting remote branch detected, pulling from remote...'
				)
			);
	...
	write(
		chalk.cyan(
			'😎 You should now be on branch: ' + chalk.underline.magenta(maint)
		)
	);
	...
	write(
		chalk.cyan(
			`When you are ready to release a patch simply run: ${chalk.bold(
				'release'
			)} from this branch. (yes, its a command 😀)`
		)
	);
	write('\n\n');
}
